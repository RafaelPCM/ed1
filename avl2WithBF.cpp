...
// https://prepinsta.com/cpp-program/deletion-in-avl-tree/
...
int balanceFactor(noBstAvl* raiz) {
    ...
    if (raiz->dir == NULL)
        ...
        alturaDireita = 1 + raiz->dir->altura;
    return(alturaEsq - alturaDireita);
}
...
struct noBstAvl* removerNo(struct noBstAvl* raiz, int val) {
    ...
    if(raiz == NULL) {
        ...
        if(val > raiz->val) {
            ...
            if(balanceFactor(raiz) >= 2)
                if (balanceFactor(raiz->esq) >= 0)
                    raiz = rebalancearEsqEsq(raiz);
                else
                    raiz = rebalancearEsqDir(raiz);
        }
        else
            if(val < raiz->val) {
                raiz->esq = removerNo(raiz->esq, val);
                if (balanceFactor(raiz) == -2)
                    if (balanceFactor(raiz->dir) <= 0)
                        raiz = rebalancearDirDir(raiz);
                    else
                        raiz = rebalancearDirEsq(raiz);
            }
            else {
                if(raiz->dir != NULL) {
                    aux = raiz->dir;
